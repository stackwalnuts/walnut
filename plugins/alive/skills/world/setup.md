...
Do NOT ask about voice or preferences in the terminal flow. Defaults are fine. The user can customize later via `/alive:config`.
...
- Import or index existing context (use `/alive:recall` after setup)
- Configure MCP integrations (use `/alive:config`)
- Set up voice customization in terminal paths (use `/alive:config`)
- Create the walnut.world link (use `/alive:config`)
