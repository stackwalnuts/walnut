...
- **Previous system detected** (v3/v4 `_brain/` folders exist) → offer migration via `/walnut:create` migrate mode
...
- **Number** → open that walnut (invoke `walnut:load`)
- **"just chat"** → freestyle conversation, no walnut focus
- **"tidy"** → invoke `walnut:tidy`
- **"find X"** → invoke `walnut:find`
- **"history"** → invoke `walnut:history`
- **"map"** → invoke `walnut:map`
- **"mine"** → invoke `walnut:mine`
...
- **static_export / markdown_vault**: Don't query at dashboard — these are for `/walnut:history` and `/walnut:find`.
